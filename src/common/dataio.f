...
C
C   schumacher, cira  may 23 2012 added line to handle bad iostat
C                                 value in readNext (search "Schumacher")
C                                 for explanation
C
C........................END PROLOGUE..................................
C
...
c     I keep getting a bad iostat value (5008) when hitting end of file, 
c     which is messing up the rest of the code.  This issue is only
c     occurring when compiling with gfortran.  I am 
c     adding this line to handle this exception so code can be 
c     implemented on NESDIS operational linux with gfortran.
c     -Andrea Schumacher, 5/23/13
...
c     skip dummy2 field
      ...
      col = col + 6
      eRcd%track%comments = recrd%remainder(col:col+51)
C
      END
...
c     skip dummy field
      col = col + 4
      read( recrd%remainder(col:col+2), '(i3)' ) eRcd%intens%rad
      col = col + 5
      eRcd%track%comments = recrd%remainder(col:col+51)
C
      END
...
c     Process the rest of the record.
      if( errRcd%trackProb ) then
         ...
         call outputRIProb( temp, errRcd%track )
         line = line(1:LEN_TRIM(line))//temp(1:LEN_TRIM(temp))
         write( datFile,'( a )') line(1:LEN_TRIM(line))
      endif
C
...
cx    write( lineEnd, '(" ",i3,",")' ) intens%v
      write( lineEnd, '(" ",i3,",   , ",i4)' ) intens%v, intens%rad

c     Add the comments.
      write( temp, '(" ",a)' ) intens%comments
      lineEnd = lineEnd(1:LEN_TRIM(lineEnd))//temp(1:LEN_TRIM(temp))
...
c
      write( lineEnd, '(" ",i3,",")' ) ri%deltaV

c     Add the comments.
      write( temp, '(" ",a)' ) ri%comments
      lineEnd = lineEnd(1:LEN_TRIM(lineEnd))//temp(1:LEN_TRIM(temp))
