#!/bin/sh
#
# grb2pack.sh - Dumps specified GRIB fields to bin files, 
#               that are then converted to pack format
#

gribFile=$GFS_FILE
gribDate=${gribFile:20}
gribTime=${gribFile:5:2}
gribFTime="0"${gribFile:15:2}
...
for ((i = 0; i < ${#patterns[@]}; i++))
  ...
  $WGRIB -s $gribFile | egrep "${patterns[$i]}" | $WGRIB $inputOptions $gribFile $outputOptions $binFile 

done

# Combine binary files to pack file
$BIN2PACK $gribDate $gribTime $gribFTime
